...
HS.fit.config <- cfa(HS.mod.2, 
                 data = HolzingerSwineford1939, 
                 group = "sex",
                 std.lv=TRUE) # to identify scale through UVI constraint

summary(HS.fit.config, standardized = TRUE, fit.measures = TRUE)
...
modificationIndices(HS.fit.scalar, free.remove=FALSE)
modificationIndices(HS.fit.scalar, free.remove=FALSE, sort. = TRUE)[1:20,]
# -> intercept x4?
